...
function intervalloDate() {
  ...
  document
    ...
    .addEventListener("change", intervalloDate);
  ...
  if (checkInValue && checkOutValue) {
    ...
    if (checkOutValue == checkInValue) {
      alert(
        "La data di check-in e quella di check-out non possono coincidere. Seleziona una data valida."
      );
      return;
    }
    ...
    document.getElementById("notti").textContent = ` x ${notti} ${
      notti === 1 ? "notte" : "notti"
    }`; //appare notti se è più di una, altrimenti notte
    document.getElementById(
      "costoNottiTotale"
    ).textContent = `${costoNottiTotale}€`;
...
nomeRows.forEach((row) => {
  ...
  function aggTestoInput() {
    ...
    if (totalePersone > 0)
      parts.push(
        `${totalePersone} ${totalePersone === 1 ? "ospite" : "ospiti"}`
      );
    if (animali > 0)
      parts.push(`${animali} ${animali === 1 ? "animale" : "animali"}`);
    if (neonati > 0)
      parts.push(`${neonati} ${neonati === 1 ? "neonato" : "neonati"}`);
